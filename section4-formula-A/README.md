...
Audit of the explicit constant chain behind **Theorem 13** (Formula (A)): the sector bounds of
Theorem 9 (checked against direct numerical differentiation of the log-density kernel, at real and
complex arguments), the two summed constants `A_V` (Theorem 10) and `F` (Theorem 12), and the
assembled error bound `E(N)` (monotonicity, `E(17) < 1`, and the asymptotic limit
`sqrt(N)*E(N) -> 0.742358`).
